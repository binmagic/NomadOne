...
prompts/: 分析/规划/生图/套图提示词；规划提示词在详情页数量为 0 时只要求头图，且 visualPrompt 必须逐字引用该模块 title/copy；visual-prompt-rewrite.ts 按当前文案重写单模块双语 visualPrompt；生图提示词默认图内字以 title/copy 为准，第一张头图可锁参考图标题字体
