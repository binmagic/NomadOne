...
user-service.ts: 首个 OWNER 锁定不可改；管理员可创建成员（不依赖开放注册开关）；登录拒绝禁用账号；删成员前清 studio/{userId} 与 prompts/{userId}；孤儿 Project/Provider 归户；自助注册读 AppSettings.allowRegister
