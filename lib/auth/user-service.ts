 ...
 * [OUTPUT]: 对外提供用户计数、创建 OWNER/MEMBER、登录校验、管理员增删改查与启停、孤儿数据归户；删成员前清 studio/{userId} 与 prompts/{userId}
 * [POS]: lib/auth 的用户仓储。首个账号即 OWNER 且不可改；管理员可创建成员，无需开放注册；删成员前清 studio 与 prompts 磁盘
...
import { deletePromptUserFiles, deleteStudioUserFiles } from "@/lib/storage/asset-manager";
...
export async function deleteManagedUser(id: string) {
  ...
  await deletePromptUserFiles(id);
